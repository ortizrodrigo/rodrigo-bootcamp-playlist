...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (!tokenResponse.ok) {
            ...
            return res.status(500).json({ error: 'Failed to get Spotify token', details: errorText });
        }
        ...
        if (!searchResponse.ok) {
            ...
            return res.status(500).json({ error: 'Spotify search failed', details: errorText });
        }
